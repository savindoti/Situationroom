...
export function Backlog() {
  ...
  const backlogTasks = tasks.filter(t => t.status === 'Resolved').sort((a, b) => b.createdAt - a.createdAt);
        ...
        <ul className="w-full flex flex-col gap-3 h-full">
           {backlogTasks.map(t => (
             <li key={t.id} className="text-sm border-b pb-3 last:border-0 border-gray-200 dark:border-slate-700">
                ...
                <div className="text-[10px] font-mono text-gray-400 dark:text-gray-500 mt-2 uppercase flex justify-between">
                  <span>Resolved</span>
